...
public class EntityMovement : MonoBehaviour
{
    ...
    public virtual void Move(Vector2 movementDir)
    {
        if (canMove)
        {
            ...
            Vector3 movement = new Vector2(Mathf.Sign(speedDif.x) * Mathf.Pow(Mathf.Abs(speedDif.x * accelRate.x), velPower) * compiledSpeedMult, Mathf.Sign(speedDif.y) * Mathf.Pow(Mathf.Abs(speedDif.y * accelRate.y), velPower) * compiledSpeedMult);
            // apply force
            // rb.AddForce(movement * Time.deltaTime);
